    ...
    public function pay(Request $request, Order $order){
        ...
        DB::transaction(function() use($order,$child){
            ...
            foreach($order->items as $item){
                abort_if(! $item->learningPath || $item->learningPath->trashed() || ! $item->learningPath->is_published, 422, 'Salah satu kelas pada pesanan sudah tidak tersedia. Hubungi admin sebelum melanjutkan pembayaran.');
                Enrollment::updateOrCreate(
                    ['child_profile_id'=>$child->id,'learning_path_id'=>$item->learning_path_id],
                    ['order_item_id'=>$item->id,'status'=>'active','enrolled_at'=>now(),'expires_at'=>null]
                );
            }
        });
        return redirect()->route('my-courses.index')->with('success','Pembayaran demo berhasil. Pendaftaran kelas sudah aktif. Silakan pilih jadwal tatap muka.');
    }
}
