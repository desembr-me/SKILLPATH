@extends('layouts.app')
@section('title','Course | SKILLPATH')
@section('content')
<section class="explore-hero"><div class="container"><span class="eyebrow">Marketplace Course</span><h1>Temukan course yang cocok untuk anak.</h1><p>Filter berdasarkan kategori, usia, kata kunci, dan urutan course.</p></div></section>
...
<div class="filter-search"><label for="q">Cari course</label><input id="q" name="q" value="{{ request('q') }}" placeholder="Coding, gitar, bahasa, menggambar..."></div>
...
<div class="explore-summary"><strong>{{ $paths->count() }}</strong><span>course ditemukan</span></div>
...
<div class="path-meta"><span>Usia {{ $path->min_age }}–{{ $path->max_age }}</span><span>{{ $path->modules_count }} modul</span></div>
...
@empty<article class="empty-card"><h2>Course tidak ditemukan.</h2><p>Ubah filter pencarian Anda.</p></article>@endforelse
</div></div></section>
@endsection
