...
<div class="course-thumb large"><span>{{ $learningPath->icon }}</span><small>{{ strtoupper($learningPath->classTypeLabel()) }}</small></div>
...
@if($isEnrolled)<a class="btn btn-dark btn-full" href="{{ route('live.index',['course'=>$learningPath->id]) }}">Lihat Jadwal Kelas</a>
...
<ul class="purchase-features"><li>✓ {{ $learningPath->modules->count() }} rangkaian kegiatan</li><li>✓ {{ $learningPath->duration_minutes }} menit durasi program</li><li>✓ {{ $learningPath->venue_summary ?: 'Lokasi mengikuti jadwal' }}</li><li>✓ {{ $learningPath->certificate_enabled ? 'Sertifikat penyelesaian' : 'Tanpa sertifikat' }}</li><li>✓ {{ $learningPath->classTypeLabel() }} tatap muka</li></ul>
</aside></div></section>
<section class="section"><div class="container course-content-grid"><div>
<div class="content-card"><h2>Keterampilan yang dikembangkan</h2><p>{{ $learningPath->learning_outcomes ?: 'Anak mengembangkan skill melalui latihan dan kegiatan praktik langsung bersama pengajar.' }}</p></div>
<div class="content-card"><h2>Rangkaian kegiatan</h2>@foreach($learningPath->modules as $module)<details class="curriculum-item" @if($loop->first) open @endif><summary><strong>{{ $loop->iteration }}. {{ $module->title }}</strong><span>{{ $module->activities->count() }} kegiatan</span></summary><p>{{ $module->summary }}</p><ul>@foreach($module->activities as $a)<li>{{ $a->title }} <small>{{ strtoupper($a->type) }}</small></li>@endforeach</ul></details>@endforeach</div>
<div class="content-card"><h2>Jadwal kelas tatap muka</h2>@forelse($learningPath->classSessions->where('status','scheduled')->where('starts_at','>=',now()) as $s)<div class="schedule-row"><div><strong>{{ $s->title }}</strong><span>{{ $s->starts_at->format('d M Y, H:i') }} - {{ $s->ends_at->format('H:i') }} · {{ $s->venue_name }}</span></div><span>{{ $s->capacity }} kursi</span></div>@empty<p>Jadwal berikutnya akan diumumkan pengajar.</p>@endforelse</div>
<div class="content-card"><h2>Ulasan peserta</h2>@auth @if($isEnrolled)<form method="POST" action="{{ route('reviews.store',$learningPath) }}" class="form-stack review-form">@csrf<label><span>Rating</span><select name="rating" required><option value="5">5 - Sangat baik</option><option value="4">4 - Baik</option><option value="3">3 - Cukup</option><option value="2">2 - Kurang</option><option value="1">1 - Sangat kurang</option></select></label><label><span>Ulasan</span><textarea name="review" rows="3" maxlength="1000" placeholder="Ceritakan pengalaman mengikuti kelas..."></textarea></label><button class="btn btn-ghost" type="submit">Simpan Ulasan</button></form>@endif @endauth @forelse($learningPath->reviews as $review)<div class="review-row"><strong>{{ $review->user->name }}</strong><span>{{ str_repeat('★',$review->rating) }}</span><p>{{ $review->review }}</p></div>@empty<p>Belum ada ulasan.</p>@endforelse</div>
@auth @if($isEnrolled)<div class="content-card"><h2>Tanya pengajar</h2><form method="POST" action="{{ route('questions.store',$learningPath) }}" class="form-stack">@csrf<textarea name="question" rows="3" placeholder="Tulis pertanyaan tentang kelas, lokasi, atau perlengkapan..." required></textarea><button class="btn btn-dark" type="submit">Kirim Pertanyaan</button></form><div class="qa-list">@foreach($learningPath->questions as $q)<article><strong>{{ $q->user->name }}</strong><p>{{ $q->question }}</p>@foreach($q->answers as $a)<div class="answer-box"><small>Jawaban pengajar</small><p>{{ $a->answer }}</p></div>@endforeach</article>@endforeach</div></div>@endif @endauth
</div><aside>
...
<div class="content-card"><h3>Persyaratan</h3><p>{{ $learningPath->requirements ?: 'Pakaian yang nyaman dan perlengkapan pribadi sesuai arahan pengajar bila diperlukan.' }}</p></div>
</aside></div></section>
@endsection
