...
<section class="dashboard-page">
    ...
    <div class="panel">
        @forelse($items as $item)
            <div class="credit-row">
                ...
                <div><h3>{{ $item->schedule->course->title }}</h3><p>{{ $item->child->name }} • Hari {{ $item->schedule->day_of_week }} • {{ substr($item->schedule->start_time,0,5) }}-{{ substr($item->schedule->end_time,0,5) }}</p><small>Rp{{ number_format($item->schedule->course->price+15000,0,',','.') }} (termasuk biaya platform)</small></div>
    ...
    <div class="panel cart-summary">
        ...
        <form method="POST" action="{{ route('parent.checkout.store') }}">@csrf<button class="btn btn-primary btn-lg">Checkout <x-icon name="arrow-right" /></button></form>
    </div>
    @endif
</section>
@endsection
