    ...
    <div class="panel">
        @forelse($enrollments as $enrollment)
            ...
            <div class="exam-row">
                ...
                <div class="exam-main"><h3>{{ $enrollment->course->title }}</h3>@if($exam)<p>Passing grade {{ $exam->passing_score }} • Maksimal {{ $exam->max_attempts }} percobaan</p>@if($attempts->count())<div class="mini-tags">@foreach($attempts as $attempt)<span>Percobaan {{ $attempt->attempt_no }}: {{ $attempt->score }} • {{ ucfirst($attempt->status) }}</span>@endforeach</div>@else<small>Belum ada percobaan ujian.</small>@endif @else<small>Ujian belum tersedia.</small>@endif</div>
                <div class="exam-status">@if($enrollment->certificate)<span class="status-chip paid">Sertifikat Terbit</span><small>{{ $enrollment->certificate->certificate_no }}</small>@elseif($exam && $attempts->count() < $exam->max_attempts)<span class="status-chip pending">Retake tersedia</span>@else<span class="status-chip locked">Menunggu</span>@endif</div>
            </div>
